<x-public-layout :title="'Pengelolaan Pengaduan'" :metaDescription="'Dokumen dan laporan pengelolaan pengaduan pelayanan publik Bagian Organisasi Kota Padang'">

    {{-- PAGE HEADER --}}
    <section class="pb-4 mb-8 border-b border-gray-200/60 max-w-7xl mx-auto px-5 lg:px-8">
        <h1 class="text-[28px] lg:text-3xl font-black tracking-tight mb-2" style="color: #047857;">Pengelolaan Pengaduan</h1>
        <nav class="flex items-center gap-2 text-[12px] font-medium text-gray-500">
            ...
            <span class="text-gray-500">Pengelolaan Pengaduan</span>
        </nav>
    </section>
        ...
        <div class="max-w-7xl mx-auto px-5 lg:px-8">
            <div class="relative bg-white rounded-[2.5rem] p-10 lg:p-14 shadow-[0_4px_25px_rgb(0,0,0,0.03)] border border-gray-100 overflow-hidden" style="border: 1px solid #cbd5e1;">
                ...
                <div class="absolute bottom-0 right-0 opacity-[0.04] pointer-events-none">
                    <i class="ph-fill ph-megaphone text-[16rem]" style="color: #059669;"></i>
                </div>
                <div class="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
                    <div class="flex items-start gap-6 max-w-2xl">
                        <div class="w-20 h-20 rounded-3xl flex items-center justify-center shrink-0" style="background-color: #ecfdf5; color: #059669; border: 1px solid #a7f3d0;">
                            <i class="ph-bold ph-megaphone text-4xl"></i>
                        </div>
                        <div class="flex-1">
                            <h2 class="text-2xl lg:text-3xl font-black mb-3 tracking-tight" style="color: #047857;">Laporan Pengelolaan Pengaduan</h2>
                            <p class="text-gray-700 font-medium leading-relaxed">
                                Transparansi tindak lanjut, penyelesaian, dan pengelolaan pengaduan, aspirasi, serta konsultasi pelayanan publik di lingkungan Bagian Organisasi Pemerintah Kota Padang.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="w-full lg:w-auto p-6 rounded-3xl shadow-lg shrink-0 flex flex-col sm:flex-row lg:flex-col items-center justify-between gap-4 text-center sm:text-left lg:text-center relative overflow-hidden" style="background: linear-gradient(135deg, #059669 0%, #047857 100%); color: #ffffff;">
                        <div>
                            <span class="text-[11px] font-extrabold uppercase tracking-wider block" style="color: #a7f3d0;">Layanan Aduan & Aspirasi</span>
                            <span class="text-base font-black block text-white mt-0.5">Ajukan Laporan Baru</span>
                        </div>
                        <a href="{{ route('public.pengaduan') }}"
                           class="px-5 py-3 rounded-xl font-extrabold text-xs shadow-md hover:bg-emerald-50 transition-all text-emerald-800 bg-white inline-flex items-center gap-2">
                            <i class="ph-bold ph-chat-text text-emerald-600 text-sm"></i>
        ...
        <div class="max-w-7xl mx-auto px-5 lg:px-8">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8 pb-4 border-b border-gray-100">
                <h3 class="text-xl font-black flex items-center gap-3 shrink-0" style="color: #047857;">
                    ...
                    <span>Dokumen Laporan Pengelolaan</span>
                </h3>

                {{-- FILTER & SEARCH BAR --}}
                <form action="{{ route('public.pengelolaan-pengaduan') }}" method="GET" class="flex flex-wrap sm:flex-nowrap items-center gap-3 sm:gap-3.5">
                    ...
                    @if(request()->filled('search') || (request()->filled('year') && request('year') != 'all'))
                        <a href="{{ route('public.pengelolaan-pengaduan') }}" class="px-4 py-2 rounded-xl text-xs font-bold bg-gray-100 text-gray-600 hover:bg-gray-200 border border-gray-200 transition-colors flex items-center justify-center gap-1 shrink-0" title="Reset Filter">
                                ...
                                <div class="flex items-center justify-between gap-4 mb-6 pt-2 relative z-10">
                                    ...
                                    <div class="flex flex-col items-end gap-1">
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black tracking-wide shadow-sm" style="background-color: #ecfdf5; color: #047857; border: 1px solid #a7f3d0;">
                                            <i class="ph-fill ph-check-circle text-emerald-600"></i> Laporan Pengaduan
                                        </span>
                                        @if($doc->year)
            ...
            @else
                <div class="bg-white rounded-[2.5rem] p-16 text-center shadow-sm border border-gray-200" style="border: 1px solid #cbd5e1;">
                    <div class="w-24 h-24 mx-auto rounded-full flex items-center justify-center mb-6" style="background-color: #ecfdf5; border: 1px solid #a7f3d0;">
                        <i class="ph-duotone ph-megaphone text-5xl" style="color: #059669;"></i>
                    </div>
                    @if(request()->filled('search') || (request()->filled('year') && request('year') != 'all'))
                        <h4 class="text-xl font-black text-gray-700 mb-2">Tidak Ada Dokumen Ditemukan</h4>
                        <p class="text-sm text-gray-500 font-medium max-w-md mx-auto mb-6">Tidak ada dokumen Laporan Pengelolaan Pengaduan yang sesuai dengan kata kunci pencarian atau filter tahun yang dipilih.</p>
                        <a href="{{ route('public.pengelolaan-pengaduan') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs shadow-md transition-all duration-300 hover:opacity-90" style="background-color: #059669; color: #ffffff;">
                            <i class="ph-bold ph-arrow-counter-clockwise"></i>
                            <span>Reset Pencarian</span>
                        </a>
                    @else
                        <h4 class="text-xl font-black text-gray-700 mb-2">Belum Ada Data Pengelolaan Pengaduan</h4>
                        <p class="text-sm text-gray-500 font-medium max-w-md mx-auto">Data Pengelolaan Pengaduan akan ditampilkan sesuai database setelah diisi oleh admin.</p>
                    @endif
                </div>
            @endif
        </div>
