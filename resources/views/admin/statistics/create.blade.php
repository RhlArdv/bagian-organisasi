<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-2xl text-gray-900 tracking-tight">Tambah Statistik</h2>
    </x-slot>
        ...
        <form action="{{ route('statistics.store') }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 border border-gray-100 shadow-[0_4px_25px_rgb(0,0,0,0.03)]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Contoh: Perangkat Daerah" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all font-medium" required>
                    </div>
                    <div>
                        <label for="value" class="block text-sm font-bold text-gray-700 mb-2">Nilai / Angka <span class="text-red-500">*</span></label>
                        <input type="text" name="value" id="value" value="{{ old('value') }}" placeholder="Contoh: 47 atau 1.482" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all font-medium" required>
                    </div>
                    <div>
                        ...
                        <input type="text" name="icon" id="icon" value="{{ old('icon', 'ph-buildings') }}" placeholder="Contoh: ph-buildings" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all font-medium" required>
                        <p class="text-xs text-gray-400 mt-1">Gunakan nama class dari Phosphor Icons (misal: ph-users, ph-buildings, dll)</p>
                    </div>
                    <div>
                        ...
                        <select name="color" id="color" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all font-medium" required>
                            <option value="brand" {{ old('color') == 'brand' ? 'selected' : '' }}>Brand (Orange)</option>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="number" name="order" id="order" value="{{ old('order', 0) }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all font-medium" required>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-3 bg-brand-500 text-white font-bold rounded-xl shadow-lg shadow-brand-500/30 hover:bg-brand-600 transition-colors flex items-center gap-2">
                    <i class="ph-bold ph-plus"></i> Simpan Statistik
                </button>
            </div>
        </form>
    </div>
