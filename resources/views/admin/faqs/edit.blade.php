<x-app-layout>
    <x-slot name="header">
        <h2 class="font-black text-2xl text-gray-900 tracking-tight">Edit FAQ</h2>
    </x-slot>
    ...
    <div class="max-w-4xl space-y-6">
        ...
        <form action="{{ route('faqs.update', $faq) }}" method="POST" class="bg-white rounded-3xl p-8 border border-gray-100 shadow-[0_4px_25px_rgb(0,0,0,0.03)]">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="question" value="{{ old('question', $faq->question) }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all font-medium" placeholder="Tuliskan pertanyaan" required>
                    @error('question')
                ...
                <div>
                    ...
                    <textarea name="answer" rows="5" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all font-medium" placeholder="Tuliskan jawaban dari pertanyaan di atas" required>{{ old('answer', $faq->answer) }}</textarea>
                    @error('answer')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="order_index" value="{{ old('order_index', $faq->order_index) }}" min="1" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all font-medium" required>
                    ...
                    <div class="pt-8 border-t border-gray-100 md:border-t-0 md:pt-0">
                        <div x-data="{ is_active: {{ old('is_active', $faq->is_active) ? 'true' : 'false' }} }" class="flex items-center gap-3 cursor-pointer group h-full md:mt-4" @click="is_active = !is_active">
